...
import { type AskExecutionResult, type AskInput, executeAsk } from "../../src/ask/structured";
...
function runTimedQuestion(params: AskInput, context: AgentToolContext): Promise<AskExecutionResult> {
	const session = {
		hasUI: true,
		settings: {
	...
	return executeAsk(createSessionAskOwner(session, context), params);
}

describe("Structured question timeout", () => {
	beforeAll(async () => {
