/**
 * Regression: an ephemeral side-channel turn must not crash with
...
// truncated final message (the crash the reporter observed on the side turn).
...
describe("session_stop block continuation — ephemeral turn resilience (#4323)", () => {
	let session: AgentSession;
	let tempDir: string;
	const authStorages: AuthStorage[] = [];

	beforeEach(() => {
		tempDir = path.join(os.tmpdir(), `pi-4323-ephemeral-${Snowflake.next()}`);
	...
	it("runEphemeralTurn recovers from a provider 'done' event whose message.content is undefined", async () => {
		...
		const agent = new Agent({
			...
			// Main-loop stream is irrelevant here; the side turn uses sideStreamFn.
		...
		const result = await session.runEphemeralTurn({ promptText: "Summarize the session." });
