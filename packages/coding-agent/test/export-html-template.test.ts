...
const expectedTemplate: TemplateProbeResult = {
	chars: 96_567,
	bytes: 96_611,
	sha256: "499a0823682777051ed17d8bab30e24f64b430510d3ff7ee7e62874ed03f236f",
	stableCache: true,
	assetsRemoved: 0,
};
...
describe("HTML export template", () => {
	...
	test("composes the exact source template once and returns the cached bytes", async () => {
		...
		expect(first).toContain("Removed tool:");
		expect(first).not.toContain("<omp-tool-view");
	...
	test("production normal bundle packs every HTML export asset", async () => {
		...
		const packedAssets = Array.from(
			Bun.stripANSI(stdout).matchAll(/^packed \S+ (dist\/template-[^.]+\.(?:css|html|js))$/gm),
			match => match[1],
		);
		expect(packedAssets).toHaveLength(3);
