...
const createSession = async (
	...
	(
		await createAgentSession({
			...
			settings: Settings.isolated({ "python.kernelMode": "session", "eval.py": true }),
			model: getModel(),
	...
	it("does not dispose unrelated Python owners when createAgentSession fails after session construction", async () => {
		...
		await expect(
			createAgentSession({
				...
				agentDir: createAgentDir(),
				sessionManager: SessionManager.inMemory(cwd),
				settings: Settings.isolated({ "python.kernelMode": "session", "eval.py": true, "memory.backend": "local" }),
				model: getModel(),
				disableExtensionDiscovery: true,
				skills: [],
				rules: [],
