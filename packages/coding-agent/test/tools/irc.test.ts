...
describe("IRC", () => {
	...
	describe("HubTool", () => {
		...
		it("op=list and op=send keep terminal transcript-only refs out of peer messaging", async () => {
			...
			const listedDetails = listed.details as CoordinationDetails | undefined;
			const peerIds = listedDetails?.peers?.map(peer => peer.id) ?? [];
			...
			expect(sentDetails?.receipts).toEqual([
				{
					to: "0-AbortedTranscript",
					outcome: "failed",
