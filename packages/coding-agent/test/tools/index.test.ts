...
describe("createTools", () => {
	...
	it("excludes todo from yield sessions unless prewalk is armed", async () => {
		...
		const settings = createSettingsWithOverrides({ "todo.enabled": true });
		const subagent = await createTools(createTestSession({ requireYieldTool: true, settings }));
		expect(subagent.map(t => t.name)).not.toContain("todo");

		const prewalkSubagent = await createTools(
			createTestSession({ requireYieldTool: true, prewalkArmed: true, settings }),
		);
