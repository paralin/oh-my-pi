...
function createSession(options: { manager?: AsyncJobManager; settings?: Record<string, unknown> }): ToolSession {
	return {
		cwd: "/tmp",
		hasUI: false,
		settings: Settings.isolated({ ...(options.manager ? { "async.enabled": true } : {}), ...options.settings }),
		getSessionFile: () => null,
		getSessionSpawns: () => "*",
...
describe("task spawn routing", () => {
	...
	it("omits history URI when an aborted background task has no retained transcript", async () => {
		...
		const result = await tool.execute("tc-abort", {
			agent: "task",
			name: "UnavailableAbort",
			task: "Start and then abort before a transcript exists.",
		} as TaskParams);
	...
	it("keeps history URI when an aborted background task retains a transcript ref", async () => {
		...
		const result = await tool.execute("tc-retained-abort", {
			agent: "task",
			name: "RetainedAbort",
			task: "Start, persist a transcript, then abort.",
		} as TaskParams);
		const job = manager.getJob(result.details!.async!.jobId)!;
		await job.promise;
