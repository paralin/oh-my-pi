...
describe("IPython agent family services", () => {
	...
	test("preserves stable IDs and family names through the World backend", async () => {
		const worldMessages: IrcMessage[] = [];
		const worldRef = {
			id: "world-child",
		...
		const backend = {
			kind: "world",
			listPeers: async () => ({ peers: [worldRef], errors: [] }),
		...
		expect(worldMessages).toEqual([
			expect.objectContaining({ id: "agentmsg_world", from: MAIN_AGENT_ID, to: "world-child" }),
		]);
	});
});
