	...
	test("runs the callable RLM ABI through typed Task admission handlers", async () => {
		...
		const phase47Handlers: IpythonHostHandlers = Object.fromEntries(
			[
				...
				"world.session_input",
				"world.session_interrupt",
				"world.dispatch_watch",
			].map(type => [
				type,
				(request: IpythonHostRequest) => {
					...
					return { kind: "ok", op: type.slice("world.".length), snapshot: null };
				},
			]),
		);
		...
		try {
			...
			expect(discovered.world).toBe(true);
			...
			expect(discovered.world_functions).toEqual([
				"dispatch_submit",
				"dispatch_watch",
				"question_answer",
				"session_input",
			...
			const phase47Result = await provisioner.execute(
				[
					...
					"submitted = await omp.world.dispatch_submit('objective', '/tmp/worktree', '/tmp/worktree', 'branch', ['result.txt'], ['src'])",
					"answered = await omp.world.question_answer('request-1', 'Proceed?', 'Yes')",
					"steered = await omp.world.session_input('request-2', 'session-1', 'continue')",
					"interrupted = await omp.world.session_interrupt('request-3', 'session-1', reason='done')",
					"watched = await omp.world.dispatch_watch('intent-1')",
					"'|'.join(map(str, (listed['jobs'][0]['id'], updated['job']['prompt'], deleted['deleted'], tools[0]['name'], resources['resources'][0]['uri'], prompts[0]['name'], 'denied' in tool_error, templates[0]['uriTemplate'], notification['params']['id'], refreshed['connection'], submitted['op'], answered['op'], steered['op'], interrupted['op'], watched['op'])))",
				].join("\n"),
			...
			expect(phase47Result.result).toBe(
				"'cron-1|updated|True|echo|demo://one|summarize|True|demo://{id}|1|connected|dispatch_submit|question_answer|session_input|session_interrupt|dispatch_watch'",
			);
			...
			expect(phase47Calls[11]?.data).toMatchObject({
				type: "world.dispatch_submit",
				objective: "objective",
				worktree_path: "/tmp/worktree",
			});
			expect(phase47Calls[15]?.data).toEqual({
				type: "world.dispatch_watch",
				intent_key: "intent-1",
				stop: "terminal",
			});
