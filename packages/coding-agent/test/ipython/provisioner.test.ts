	...
	test("runs the callable RLM ABI through typed Task admission handlers", async () => {
		...
		try {
			...
			expect(discovered.capabilities).toEqual(
				expect.arrayContaining(["omp.workspace", "omp.files", "omp.code", "omp.debug", "edit", "attach_image"]),
			);
