...
import bossSystemPrompt from "./boss-system.md" with { type: "text" };
...
export function resolveBossScratchHandoffFile(input: {
	...
	if (input.explicitScratchFile?.trim()) return input.explicitScratchFile;
...
function registerBossProvider(pi: Parameters<ExtensionFactory>[0]): void {
	...
	if (!codex?.baseUrl || !codex.api) return;
...
async function loadGladosBossSkillContext(): Promise<string | undefined> {
	...
	if (gladosBossSkillContextCache?.key === cacheKey) return gladosBossSkillContextCache.value;
...
export const createGladosBossExtension: ExtensionFactory = pi => {
	...
	pi.on("session_start", async (_event, ctx) => {
		if (pi.getFlag(GLADOS_BOSS_FLAG) !== true) return;
		...
		if (!pi.getSessionName()) await pi.setSessionName(`Boss: ${ctx.cwd}`);
	...
	pi.on("before_agent_start", async event => {
		...
		return {
			systemPrompt: skillContext
				? [...event.systemPrompt, bossSystemPrompt, skillContext]
				: [...event.systemPrompt, bossSystemPrompt],
		};
	});
};
