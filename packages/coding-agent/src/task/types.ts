...
export function getTaskSchema(options: {
	...
	if (defaultAgent === "task" && !effortEnabled) {
		if (options.batchEnabled) {
			return options.isolationEnabled ? (taskSchemaBatch as BaseType) : (taskSchemaBatchNoIsolation as BaseType);
		}
		return options.isolationEnabled ? (taskSchema as BaseType) : (taskSchemaNoIsolation as BaseType);
	}
