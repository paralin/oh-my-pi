...
async function runInstall(dirOverride: string | undefined): Promise<void> {
	...
	console.log("");
	console.log("omp starts the relay automatically when the typed browser host capability needs it;");
...
async function runServe(args: BrowserRelayCommandArgs): Promise<void> {
	...
	const readiness = setInterval(() => {
		if (relay.bridge.ready && !announced) {
			announced = true;
			console.log("Extension connected. The omp.browser host capability can now drive your tabs.");
