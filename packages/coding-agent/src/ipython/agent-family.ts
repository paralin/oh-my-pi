...
export class OmpAgentFamilyService implements AgentFamilyService {
	...
	async #snapshot(signal?: AbortSignal): Promise<AgentFamilySnapshot> {
		...
		const worldRoster = this.#coordinationBackend ? await this.#coordinationBackend.listPeers(signal) : undefined;
		const errors = [...(worldRoster?.errors ?? [])];
		...
		for (const node of nodes.values()) {
			...
			entries.push({
				...
				status: node.status,
				...(node.source === "world" ? { source: "world" as const } : {}),
			});
		}
		entries.sort(
			(left, right) =>
	...
	async #sendOne(
		...
		const outbound: IrcMessage = {
			...
			ts: Date.now(),
			...(replyTo ? { replyTo } : {}),
			...(target.source === "world" ? { source: "world" as const } : {}),
		};
		if (target.source === "world") {
			if (!this.#coordinationBackend) throw new Error("World coordination backend is unavailable");
			...
			return receiptFor(
				...
				{ messageObjectKey: receipt.messageObjectKey, inboxSequence: receipt.inboxSequence.toString() },
			);
		}
	...
	async #takeInbox(
		...
		for (const message of this.#coordinationBackend?.inbox({
			...
			tagged.push({ message, source: "world" });
		}
		...
		if (request.consume && selected.length > 0) {
			...
			const worldCount = selected.filter(item => item.sources.has("world")).length;
			if (worldCount > 0) {
