...
export class IpythonController {
	...
	#nextId = 0;
	#generation = 0;
	#comms = new Map<string, { readonly executionId: string; readonly targetName: string }>();
	...
	async #dispatch(frame: ControllerFrame, generation: number): Promise<void> {
		...
		if (frame.event === "comm") {
			...
			if (frame.operation === "open") {
				...
				this.#comms.set(frame.comm_id, { executionId: frame.id, targetName: frame.target_name });
			}
	...
	async #handleComm(frame: Extract<ControllerFrame, { event: "comm" }>, generation: number): Promise<void> {
		const active = this.#active;
		if (generation !== this.#generation || active?.id !== frame.id) return;
		...
		try {
			...
			const handler = this.#options.hostHandlers?.[operation];
			if (!handler) throw new IpythonControllerError(`unknown host request type: ${operation}`);
		...
		if (generation !== this.#generation) return;
		const comm = this.#comms.get(frame.comm_id);
		if (!comm || comm.executionId !== frame.id) return;
		await this.#write({ op: "comm_reply", comm_id: frame.comm_id, data: response }, generation);
		if (this.#comms.get(frame.comm_id) === comm) this.#comms.delete(frame.comm_id);
	}
	...
	async #handleExit(
		...
		if (!this.#disposed && processIds) {
			try {
				process.kill(processIds.kernelPid, "SIGKILL");
			} catch {
	...
	async #disposeInternal(): Promise<void> {
		...
		if (await this.#exitedWithin(proc, this.#options.shutdownGraceMs ?? SHUTDOWN_GRACE_MS)) return;
		try {
			proc.kill("SIGTERM");
		} catch {
			// The process may have exited between the wait and the signal.
		}
		if (await this.#exitedWithin(proc, SHUTDOWN_ESCALATION_MS)) return;
		try {
			proc.kill("SIGKILL");
		} catch {
			// The process may have exited between the wait and the signal.
