...
export class HistoryProtocolHandler implements ProtocolHandler {
	...
	async resolve(url: InternalUrl): Promise<InternalResource> {
		...
		if (!ref) {
			// A transcript that identifies a parent session is eligible for persisted
			// subagent reconstruction. Other artifact JSONL files remain explicitly
			// disk-only even if they happen to sit under a registered artifact dir.
			const disk = await this.#resolveFromDisk(agentId);
			if (disk && !(await this.#hasParentSession(disk.sourcePath))) return { ...disk, url: url.href };
			...
			if (!ref && disk) return { ...disk, url: url.href };
		}
		if (!ref) {
	...
	async #hasParentSession(sessionFile: string | undefined): Promise<boolean> {
		if (!sessionFile) return false;
		try {
			const entries = await loadEntriesFromFile(sessionFile);
			return entries.some(entry => entry.type === "session" && Boolean(entry.parentSession));
		} catch {
			return false;
		}
	}
