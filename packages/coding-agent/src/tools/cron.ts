...
import { createCachedComponent, formatMoreItems, PREVIEW_LIMITS, replaceTabs } from "./render-utils";
...
function cronJobRow(job: CronJob, theme: Theme): string {
	return [
		theme.fg("accent", replaceTabs(job.expression)),
		theme.fg("dim", job.id),
	...
	return {
			...
			const header = renderStatusLine(
				{
					icon: "pending",
					title: `Cron ${op}`,
					description: op === "create" ? args.expression : op === "delete" ? args.id : undefined,
					meta,
				},
				theme,
			);
		...
		renderResult(
			...
			const text = result.content
				...
				.trim();
			const body: string[] = [];
			let description = op === "create" ? args?.expression : op === "delete" ? args?.id : undefined;
			let meta: string[] = [];
			let icon: "error" | "warning" | undefined;

			if (result.isError) {
				icon = "error";
				body.push(theme.fg("error", replaceTabs(text || "Unknown error")));
			} else if (op === "create" && isCronJob(details)) {
				description = details.expression;
				meta = [theme.fg("dim", `next ${new Date(details.nextFireAt).toLocaleString()}`), ...cronMode(details)];
				body.push(`${theme.fg("accent", details.id)} ${theme.fg("dim", replaceTabs(details.prompt))}`);
				...
				body.push(theme.fg("toolOutput", replaceTabs(text)));
			}
