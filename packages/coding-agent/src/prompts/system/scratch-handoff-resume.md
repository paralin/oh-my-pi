...
{{#if scratchTruncated}}Only checkpoint beginning is injected. Use one `ipython {code}` cell with `pathlib.Path` to read `{{displayPath}}` only if active TODO, next action, or a required referenced detail is missing below.{{else}}Do not reread `{{displayPath}}`; full checkpoint is supplied.{{/if}}
