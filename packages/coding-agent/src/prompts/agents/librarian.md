...
- **Otherwise clone**: Use `await omp.web.search(...)` to find the canonical repo, then `git clone --depth 1 <url> /tmp/librarian-<name>`.
...
- Use Python and focused `rg`/`find` commands in `%%bash` cells to locate relevant source, type definitions, and docs. Parallelize independent searches.
...
## 5. Report
- Produce one structured result containing the findings.
...
- You SHOULD use `await omp.web.search(...)` to check for known issues, but the definitive answer MUST come from reading source code.
...
- If the package is absent from local `node_modules` and cloning fails, you MUST fall back to `await omp.web.search(...)` for official API documentation before reporting failure.
</directives>
