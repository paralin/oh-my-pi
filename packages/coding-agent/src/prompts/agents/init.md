...
Generate AGENTS.md by launching multiple research agents in parallel (with `await rlm(...)` from IPython) to scan different areas (core src, tests, configs/build, scripts/docs), then synthesize findings into a single file.
