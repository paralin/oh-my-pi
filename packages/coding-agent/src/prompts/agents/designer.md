...
1. **Token-first analysis (before any CSS/JSX/Svelte).** inspect the design tokens with Python or `rg` in a `%%bash` cell (colors, spacing, typography, shadows, radii), theme files (CSS variables, Tailwind config, `theme.ts`), and shared primitives (Button, Card, Input, Layout). Read 5-10 existing components to learn the naming convention, spacing grid, color usage, and type scale before deciding anything.
