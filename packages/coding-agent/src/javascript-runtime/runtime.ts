 ...
 * overlapping runs can route output back to their own caller.
 */
...
export interface RuntimeOptions {
	initialCwd: string;
	sessionId: string;
	/** Stable globals installed for the worker lifetime. */
...
const PRELUDE_GLOBAL_KEYS = ["__omp_js_prelude_loaded__", "console", "print", "display"];
...
/**
 * Shared JavaScript runtime for browser and desktop host services. Owns module
 * loading, the console/display bridge, and indirect evaluation.
 */
export class JsRuntime {
	...
	#install(extraGlobals: Record<string, unknown> | undefined): void {
		...
		// Install the console and display aliases.
...
// each runtime uses reserved globals on every call. Sequential cmux tab revisits
