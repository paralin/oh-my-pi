...
export async function executeRunExperimentOwner(
	options: AutoresearchOperationOptions,
	ctx: ExtensionContext,
	params: RunExperimentParams,
	signal: AbortSignal | undefined,
	onUpdate: AgentToolUpdateCallback<RunDetails | RunExperimentProgressDetails> | undefined,
): Promise<AutoresearchOperationResult<RunDetails>> {
...
function buildRunText(details: RunDetails, outputPreview: string, bestMetric: number | null): string {
	...
	if (details.parsedPrimary !== null) {
		...
		lines.push(`Next omp.autoresearch.log metric: ${details.parsedPrimary}`);
	}
	if (details.parsedMetrics) {
		...
		if (secondary.length > 0) {
			...
			lines.push(`Next omp.autoresearch.log metrics: ${JSON.stringify(Object.fromEntries(secondaryEntries))}`);
		}
	}
