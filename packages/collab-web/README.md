...
- `src/tool-render/` — generic React rendering for collaborative transcript tool records with common `ToolView` chrome and theme-adaptive `tv-` design tokens. The `ToolRenderHost` seam lets hosts wire agent-id chips to a sub-session drawer.
- `scripts/` — `local-relay.ts` (content-blind relay on `Bun.serve`) and `mock-host.ts` + `fixture.ts` (scripted host for offline development).
