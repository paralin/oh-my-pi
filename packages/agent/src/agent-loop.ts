		...
		while (true) {
			...
			while (hasMoreToolCalls || pendingMessages.length > 0) {
				...
				try {
					...
					gateResult = await config.beforeModelCall?.(preparedProviderCall.context);
				} catch (error) {
					if (!turnOpen) {
						stream.push({ type: "turn_start" });
				...
				if (gateResult?.stop) {
					...
					emitInputMessages(stream, turnMessages);
					if (!turnOpen && !signal?.aborted) config.onToolChoiceRejected?.();
					if (turnOpen) {
						const stopMessage = createGateStopMessage(config.model, gateResult.reason);
