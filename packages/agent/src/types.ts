 ...
 * (for example, after the target session closes).
 */
...
export interface AgentLoopConfig extends Omit<SimpleStreamOptions, "toolChoice"> {
	 ...
	 * message conversion, provider transforms, and normalized tools. Returning {@link AgentPreModelCallStop} ends
	...
	/**
	 * Owned in-band tool transport for models whose provider does not accept
	 * native tool definitions.
	 */
	dialect?: Dialect;
	/** Stop an owned transport when the model starts fabricating a tool result. */
	 ...
	 * byte prefix) and caches system prompt + tools.
	 */
...
export interface BeforeToolCallContext {
	 ...
	 * so in-place mutations stick; a returned `BeforeToolCallResult.args` replaces
	 * them entirely.
	 */
	args: Record<string, unknown>;
