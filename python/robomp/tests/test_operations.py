"""Operation tests against a mocked GitHub via httpx.MockTransport."""
...
from robomp import operations as operations_module
from robomp.db import Database
from robomp.github_client import GitHubClient, IssueIndexEntry, IssueInfo, RepoInfo
from robomp.operations import AbortController, OperationError, ToolBindings
from robomp.operations import operations as build_operations
...
def test_repo_command_env_scrubs_secrets_and_uses_workspace_cache(
    ...
    try:
        env = operations_module._repo_command_env(bindings)
    finally:
...
def test_run_repo_command_uses_slot_identity_kwargs(
    ...
    monkeypatch.setattr(operations_module.subprocess, "run", fake_run)  # type: ignore[attr-defined]
    try:
        proc = operations_module._run_repo_command(bindings, ["git", "status"])
    finally:
        _stop_loop(loop, thread)
...
def test_ensure_workspace_dependencies_installs_when_missing(
    ...
    monkeypatch.setattr(operations_module, "_run_repo_command", fake_run_repo_command)
    try:
        operations_module.ensure_workspace_dependencies(bindings)
    finally:
        _stop_loop(loop, thread)
...
def test_ensure_workspace_dependencies_skips_non_bun_repo(
    ...
    monkeypatch.setattr(operations_module, "_run_repo_command", fail_run)
    try:
        operations_module.ensure_workspace_dependencies(bindings)
    finally:
        _stop_loop(loop, thread)
...
def test_ensure_workspace_dependencies_swallows_install_failure(
    ...
    monkeypatch.setattr(operations_module, "_run_repo_command", failing_run)
    try:
...
def test_guarded_push_branch_rev_parse_runs_via_repo_command_and_passes_slot_uid(
    ...
    monkeypatch.setattr(operations_module, "_run_repo_command", fake_run_repo_command)
    monkeypatch.setattr(operations_module, "_share_git_metadata_with_slots", lambda _repo_dir, _slot_uid: None)
    try:
        head = operations_module._guarded_push_branch(bindings, {}, "gh_push_branch", bindings.workspace.branch)
    finally:
        _stop_loop(loop, thread)
...
def test_gh_post_comment_happy_path(db: Database, tmp_path: Path) -> None:
    ...
    try:
        tool = build_operations(bindings)["gh_post_comment"]
        result = tool.execute({"body": "hi"})
    finally:
        _stop_loop(loop, t)
...
def test_gh_post_comment_explicit_number_overrides_inbound(db: Database, tmp_path: Path) -> None:
    ...
    try:
        tool = build_operations(bindings)["gh_post_comment"]
        tool.execute({"body": "hi", "number": 42})
    finally:
        _stop_loop(loop, thread)
...
def test_gh_open_pr_requires_template_sections(db: Database, tmp_path: Path) -> None:
    ...
    try:
        tool = build_operations(bindings)["gh_open_pr"]
        with pytest.raises(OperationError) as exc:
            tool.execute({"title": "t", "body": "no sections"})
...
def test_repro_record_writes_transcript(db: Database, tmp_path: Path) -> None:
    bindings, loop, t = _bindings(db, tmp_path, httpx.MockTransport(lambda r: httpx.Response(500)))
    try:
        tool = build_operations(bindings)["repro_record"]
        result = tool.execute(
            {
...
def test_repro_record_chowns_to_slot_when_root(db: Database, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(operations_module, "_slot_permissions_active", lambda slot_uid: slot_uid is not None)
    monkeypatch.setattr(
        "robomp.operations.os.chown",
...
def test_repro_record_rejects_bad_args(db: Database, tmp_path: Path) -> None:
    ...
    try:
        tool = build_operations(bindings)["repro_record"]
        with pytest.raises(OperationError):
            tool.execute({"title": "", "command": "x", "output": "y", "exit_code": 1})
        with pytest.raises(OperationError):
            tool.execute({"title": "t", "command": "x", "output": "y", "exit_code": "bad"})
    finally:
        _stop_loop(loop, t)
...
def test_mark_unable_posts_comment_marks_needs_info_and_labels_issue(db: Database, tmp_path: Path) -> None:
    ...
    bindings, loop, t = _bindings(db, tmp_path, httpx.MockTransport(handler))
    try:
        tool = build_operations(bindings)["mark_unable_to_reproduce"]
        result = tool.execute({"diagnosis": "needed exact version", "info_needed": "post bun --version"})
    finally:
        _stop_loop(loop, t)
...
def test_mark_unable_keeps_needs_info_when_label_is_missing(db: Database, tmp_path: Path) -> None:
    ...
    try:
        tool = build_operations(bindings)["mark_unable_to_reproduce"]
        tool.execute({"diagnosis": "needed exact version", "info_needed": "post bun --version"})
    finally:
        _stop_loop(loop, t)
...
def test_abort_task_signals_controller_and_abandons_without_comment(db: Database, tmp_path: Path) -> None:
    ...
    try:
        tool = build_operations(bindings)["abort_task"]
        result = tool.execute({"reason": "ref dir owned by foreign uid; git commit cannot lock HEAD"})
    finally:
        _stop_loop(loop, t)
...
def test_fetch_issue_thread_returns_markdown(db: Database, tmp_path: Path) -> None:
    ...
    bindings, loop, t = _bindings(db, tmp_path, httpx.MockTransport(handler))
    try:
        tool = build_operations(bindings)["fetch_issue_thread"]
        result = tool.execute({})
    finally:
        _stop_loop(loop, t)
...
def test_classify_issue_applies_labels_and_persists_primary(db: Database, tmp_path: Path) -> None:
    ...
    bindings, loop, t = _bindings(db, tmp_path, httpx.MockTransport(handler))
    try:
        tool = build_operations(bindings)["classify_issue"]
        result = tool.execute(
            {
                "primary": "bug",
                "priority": "prio:p1",
...
def test_gh_search_issues_scopes_repo_and_renders_matches(db: Database, tmp_path: Path) -> None:
    ...
    bindings, loop, t = _bindings(db, tmp_path, httpx.MockTransport(handler))
    try:
        tool = build_operations(bindings)["gh_search_issues"]
        result = tool.execute({"query": "resize crash"})
    finally:
        _stop_loop(loop, t)
...
def test_gh_search_issues_rejects_repo_qualifier_and_empty_query(db: Database, tmp_path: Path) -> None:
    ...
    try:
        tool = build_operations(bindings)["gh_search_issues"]
        with pytest.raises(OperationError):
            tool.execute({"query": "repo:evil/elsewhere secrets"})
        with pytest.raises(OperationError):
            tool.execute({"query": "   "})
    finally:
        _stop_loop(loop, t)
...
def test_gh_search_issues_serves_from_local_index_once_synced(db: Database, tmp_path: Path) -> None:
    ...
    try:
        tool = build_operations(bindings)["gh_search_issues"]
        result = tool.execute({"query": "resize crash"})
        pr_only = tool.execute({"query": "resize crash is:merged"})
    finally:
        _stop_loop(loop, t)
    assert "#30 (issue, closed (not_planned))" in result
...
def test_search_commits_message_and_patch_modes(db: Database, tmp_path: Path) -> None:
    ...
    try:
        tool = build_operations(bindings)["search_commits"]
        by_message = tool.execute({"query": "colon selector"})
        by_patch = tool.execute({"query": "splitPathAndSel", "mode": "patch"})
        none = tool.execute({"query": "nonexistent-topic"})
        with pytest.raises(OperationError):
            tool.execute({"query": "x", "mode": "bogus"})
    finally:
        _stop_loop(loop, t)
...
def test_classify_issue_rejects_bug_without_priority(db: Database, tmp_path: Path) -> None:
    ...
    try:
        tool = build_operations(bindings)["classify_issue"]
        with pytest.raises(OperationError):
            tool.execute({"primary": "bug", "rationale": "yes a bug"})
    finally:
        _stop_loop(loop, t)
...
def test_classify_issue_rejects_unknown_primary(db: Database, tmp_path: Path) -> None:
    ...
    try:
        tool = build_operations(bindings)["classify_issue"]
        with pytest.raises(OperationError):
            tool.execute({"primary": "nonsense", "rationale": "x"})
    finally:
        _stop_loop(loop, t)
...
def test_fetch_pr_returns_premise_and_changed_files(db: Database, tmp_path: Path) -> None:
    ...
    try:
        tool = build_operations(bindings)["fetch_pr"]
        result = tool.execute({})
    finally:
        _stop_loop(loop, t)
...
def test_classify_pr_applies_review_labels_and_persists_rank(db: Database, tmp_path: Path) -> None:
    ...
    bindings, loop, t = _review_bindings(db, tmp_path, httpx.MockTransport(handler))
    try:
        tool = build_operations(bindings)["classify_pr"]
        result = tool.execute(
            {
                "rank": "review:p1",
                "type": "fix",
...
def test_classify_pr_rejects_bad_rank(db: Database, tmp_path: Path) -> None:
    ...
    try:
        tool = build_operations(bindings)["classify_pr"]
        with pytest.raises(OperationError):
            tool.execute({"rank": "prio:p1", "type": "fix", "rationale": "wrong namespace"})
    finally:
        _stop_loop(loop, t)
...
def test_pr_review_comment_stages_and_submit_flushes_one_comment_review(db: Database, tmp_path: Path) -> None:
    ...
    bindings, loop, t = _review_bindings(db, tmp_path, httpx.MockTransport(handler))
    try:
        stage_tool = build_operations(bindings)["pr_review_comment"]
        submit_tool = build_operations(bindings)["submit_pr_review"]
        ...
        result = submit_tool.execute({"body": "review:p1 — one blocking issue", "event": "APPROVE"})
    finally:
        _stop_loop(loop, t)
...
def test_submit_pr_review_posts_summary_only_when_no_staged_comments(db: Database, tmp_path: Path) -> None:
    ...
    bindings, loop, t = _review_bindings(db, tmp_path, httpx.MockTransport(handler))
    try:
        tool = build_operations(bindings)["submit_pr_review"]
        result = tool.execute({"body": "lgtm — scoped fix"})
    finally:
        _stop_loop(loop, t)
...
def test_submit_pr_review_failure_keeps_staged_comments(db: Database, tmp_path: Path) -> None:
    ...
    try:
        stage_tool = build_operations(bindings)["pr_review_comment"]
        submit_tool = build_operations(bindings)["submit_pr_review"]
        stage_tool.execute({"path": "src/app.py", "line": 12, "body": "finding"})
        with pytest.raises(OperationError):
            submit_tool.execute({"body": "summary"})
    finally:
        _stop_loop(loop, t)
...
def test_review_tools_reject_outside_review_mode(db: Database, tmp_path: Path) -> None:
    ...
    try:
        tool = build_operations(bindings)["pr_review_comment"]
        with pytest.raises(OperationError):
            tool.execute({"path": "x.py", "line": 1, "body": "nit"})
    finally:
        _stop_loop(loop, t)
...
def test_review_mode_rejects_push_and_open_pr_before_repo_commands(db: Database, tmp_path: Path) -> None:
    ...
    bindings, loop, t = _review_bindings(db, tmp_path, httpx.MockTransport(lambda _r: httpx.Response(500)))
    try:
        original = operations_module._run_repo_command
        operations_module._run_repo_command = record_repo_command  # type: ignore[assignment]
        push = build_operations(bindings)["gh_push_branch"]
        open_pr = build_operations(bindings)["gh_open_pr"]
        with pytest.raises(OperationError):
            push.execute({})
        with pytest.raises(OperationError):
            open_pr.execute({"title": "t", "body": "invalid"})
    finally:
        operations_module._run_repo_command = original  # type: ignore[assignment]
        _stop_loop(loop, t)
...
def test_impl_gate_rejects_unauthorized_non_auto_classification_before_repo_commands(
    ...
    monkeypatch.setattr(operations_module, "_run_repo_command", record_repo_command)
    try:
        push = build_operations(bindings)["gh_push_branch"]
        open_pr = build_operations(bindings)["gh_open_pr"]
        with pytest.raises(OperationError) as push_exc:
            push.execute({})
        with pytest.raises(OperationError) as pr_exc:
            open_pr.execute({"title": "fix: x", "body": "invalid"})
    finally:
        _stop_loop(loop, t)
...
def test_impl_gate_allows_authorized_non_auto_classification_push_to_reach_repo_commands(
    ...
    monkeypatch.setattr(operations_module, "_run_repo_command", record_repo_command)
    try:
        tool = build_operations(bindings)["gh_push_branch"]
...
def test_set_issue_labels_on_pr_thread_is_noop(db: Database, tmp_path: Path) -> None:
    ...
    bindings, loop, t = _pr_bindings(db, tmp_path, httpx.MockTransport(handler))
    try:
        tool = build_operations(bindings)["set_issue_labels"]
        result = tool.execute({"labels": ["wontfix"]})
    finally:
        _stop_loop(loop, t)
    assert "no-op" in result.lower()
    assert calls == []
...
def test_classify_issue_rename_failure_does_not_apply_labels_or_classification(
    ...
    def fail_rename(*_args: object, **_kwargs: object) -> str:
        raise operations_module.GitCommandError(["git", "branch", "-m"], 128, "", "fatal: detected dubious ownership")

    bindings, loop, t = _bindings(db, tmp_path, httpx.MockTransport(handler))
    monkeypatch.setattr(operations_module, "rename_workspace_branch", fail_rename)
    try:
        tool = build_operations(bindings)["classify_issue"]
        with pytest.raises(OperationError):
            tool.execute(
...
def test_gh_open_pr_rejects_wrong_identity_before_push_or_pr(db: Database, tmp_path: Path) -> None:
    ...
    try:
        ...
        body = "## Repro\nrepro\n\n## Cause\ncause\n\n## Fix\nfix\n\n## Verification\nran tests\n\nFixes #42\n"
        with pytest.raises(OperationError) as exc:
            tool.execute({"title": "fix: x", "body": body})
...
def test_gh_push_branch_skip_checks_bypasses_failing_bun_check(
    ...
    try:
        ...
        db.set_issue_classification(bindings.issue_key, "bug")
        tool = build_operations(bindings)["gh_push_branch"]
        result = tool.execute({"skip_checks": True})
    finally:
        _stop_loop(loop, thread)
...
def test_gh_open_pr_runs_fix_then_check_and_amends_formatter_diff(
    ...
    try:
        ...
        tool = build_operations(bindings)["gh_open_pr"]
        body = "## Repro\nrepro\n\n## Cause\ncause\n\n## Fix\nfix\n\n## Verification\nran tests\n\nFixes #42\n"
        result = tool.execute({"title": "fix: x", "body": body})
    finally:
        _stop_loop(loop, thread)
...
def test_gh_open_pr_refuses_dirty_worktree_before_fix(
    ...
    try:
        ...
        db.set_issue_classification(bindings.issue_key, "bug")
        push_tool = build_operations(bindings)["gh_push_branch"]
        with pytest.raises(OperationError) as exc:
            push_tool.execute({})
        assert "dirty worktree" in str(exc.value).lower()

        pr_tool = build_operations(bindings)["gh_open_pr"]
        body = "## Repro\nr\n\n## Cause\nc\n\n## Fix\nf\n\n## Verification\nv\n\nFixes #42\n"
        with pytest.raises(OperationError) as exc2:
            pr_tool.execute({"title": "fix: x", "body": body})
...
def test_gh_post_comment_appends_suffix_and_schedules_for_question(db: Database, tmp_path: Path) -> None:
    ...
    try:
        tool = build_operations(bindings)["gh_post_comment"]
        tool.execute({"body": "Here's the answer"})
    finally:
        _stop_loop(loop, t)
...
def test_gh_post_comment_skips_suffix_for_non_question(db: Database, tmp_path: Path) -> None:
    ...
    try:
        tool = build_operations(bindings)["gh_post_comment"]
        tool.execute({"body": "Here's the diagnosis"})
    finally:
        _stop_loop(loop, t)
...
def test_gh_post_comment_skips_suffix_when_target_differs_from_origin(db: Database, tmp_path: Path) -> None:
    ...
    try:
        tool = build_operations(bindings)["gh_post_comment"]
        tool.execute({"body": "see other issue", "number": 99})
    finally:
        _stop_loop(loop, t)
