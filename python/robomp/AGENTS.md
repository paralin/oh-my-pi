...
7. Inside the subprocess the agent receives only the exclusive `ipython` provider interface. In complete Python or `%%bash` cells scoped to the worktree, it uses the task-local typed `robomp_host` operations backed by `operations.py`; that is the only surface allowed to mutate GitHub or write audit rows.
