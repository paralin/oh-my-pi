"""Task-bound Robomp operations for GitHub, review, reproduction, and publication.
...
class OperationError(RuntimeError):
    """Rejected Robomp operation."""

    def __init__(self, command: str, error: Any = None, code: str | None = None) -> None:
        self.command = command
        self.error = error
        self.code = code
        super().__init__(command if error is None else f"{command}: {error}")
...
class AbortController:
    """Mutable handoff between the `abort_task` operation and the worker.

    `signal()` is called from the operation thread to request an irrecoverable
...
class ToolBindings:
    """Per-task closure that the operations capture."""
...
def _raise_command(message: str) -> NoReturn:
    raise OperationError(message, error={"message": message})
...
def _run_pre_publish_bun_fix(
    ...
    *,
    ...
    `tool_name` is the operation calling this (audit attribution).
...
# ---------- gh_post_comment ----------
def _build_post_comment(bindings: ToolBindings) -> Operation:
    ...
    return Operation("gh_post_comment", lambda args: execute(args))
...
# ---------- gh_push_branch ----------
def _build_push_branch(bindings: ToolBindings) -> Operation:
    def execute(args: dict[str, Any]) -> str:
        if bindings.review_mode:
            msg = "refusing to push: PR review worktrees are read-only."
    ...
    return Operation("gh_push_branch", lambda args: execute(args))


# ---------- gh_open_pr ----------
def _build_open_pr(bindings: ToolBindings) -> Operation:
    def execute(args: dict[str, Any]) -> str:
        if bindings.review_mode:
    ...
    return Operation("gh_open_pr", lambda args: execute(args))


# ---------- gh_request_review ----------
def _build_request_review(bindings: ToolBindings) -> Operation:
    def execute(args: dict[str, Any]) -> str:
    ...
    return Operation("gh_request_review", lambda args: execute(args))


# ---------- repro_record ----------
def _build_repro_record(bindings: ToolBindings) -> Operation:
    def execute(args: dict[str, Any]) -> str:
        title = args.get("title")
    ...
    return Operation("repro_record", lambda args: execute(args))


# ---------- mark_unable_to_reproduce ----------
def _build_mark_unable(bindings: ToolBindings) -> Operation:
    def execute(args: dict[str, Any]) -> str:
    ...
    return Operation("mark_unable_to_reproduce", lambda args: execute(args))


# ---------- abort_task ----------
def _build_abort_task(bindings: ToolBindings) -> Operation:
    def execute(args: dict[str, Any]) -> str:
    ...
    return Operation("abort_task", lambda args: execute(args))


# ---------- fetch_issue_thread ----------
def _build_fetch_thread(bindings: ToolBindings) -> Operation:
    def execute(args: dict[str, Any]) -> str:
        try:
            issue = _run_coro(
                bindings.loop,
    ...
    return Operation("fetch_issue_thread", lambda args: execute(args))
...
def _build_search_issues(bindings: ToolBindings) -> Operation:
    ...
    return Operation("gh_search_issues", lambda args: execute(args))
...
def _build_search_commits(bindings: ToolBindings) -> Operation:
    ...
    return Operation("search_commits", lambda args: execute(args))
...
def _build_fetch_pr(bindings: ToolBindings) -> Operation:
    def execute(args: dict[str, Any]) -> str:
    ...
    return Operation("fetch_pr", lambda args: execute(args))


def _build_classify_pr(bindings: ToolBindings) -> Operation:
    def execute(args: dict[str, Any]) -> str:
    ...
    return Operation("classify_pr", lambda args: execute(args))
...
def _build_pr_review_comment(bindings: ToolBindings) -> Operation:
    def execute(args: dict[str, Any]) -> str:
    ...
    return Operation("pr_review_comment", lambda args: execute(args))


def _build_submit_pr_review(bindings: ToolBindings) -> Operation:
    def execute(args: dict[str, Any]) -> str:
    ...
    return Operation("submit_pr_review", lambda args: execute(args))


def _build_set_issue_labels(bindings: ToolBindings) -> Operation:
    ...
    return Operation("set_issue_labels", lambda args: execute(args))


def _build_classify_issue(bindings: ToolBindings) -> Operation:
    ...
    return Operation("classify_issue", lambda args: execute(args))


def operations(bindings: ToolBindings) -> dict[str, Operation]:
    """Return the full set of operations bound to one task's context.
    ...
    return {operation.name: operation for operation in values}


__all__ = ["AbortController", "Operation", "ToolBindings", "operations"]
