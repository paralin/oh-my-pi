...
| `autolearn.enabled`      | boolean | `false` | Legacy compatibility setting. Fixed-IPython sessions do not expose `learn` or `manage_skill`; create durable records with `omp.memory` and reusable records with `omp.skills` from a cell. |
