...
Fixed-IPython sessions do not expose a `learn` tool. Create a durable memory
record with `omp.memory` and a reusable skill record with `omp.skills`:
...
sessions. They do not change the active provider prompt mid-session.
