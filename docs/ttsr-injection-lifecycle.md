...
- for tools exposing `matcherDigest` (edit/write), replace the scoped buffer with that tool's source-bearing payload — `new_text` in replace mode, `+` body rows or added diff lines in the other edit update modes, full content for a patch create, and the entire `content` for `write` — and call `checkSnapshot(snapshot, matchContext)`; otherwise append the delta into a source/tool scoped manager buffer and call `checkDelta(delta, matchContext)` (synchronous regex matching either way)
