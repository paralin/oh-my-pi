...
functions. Fixed-IPython sessions do not expose `recall`, `retain`, `reflect`,
`memory_edit`, or `learn`.

Use the `/memory` commands to inspect, diagnose, clear, or enqueue the active
backend. `omp.memory` and `omp.skills` manage host-owned harness records; they
