...
| `PI_TINY_DEVICE`            | ONNX execution provider for local tiny models; overrides the `providers.tinyModelDevice` setting (default: CPU; supports `cpu`, `cuda`, `dml`, `coreml`, `gpu`, `metal`/`webgpu`, `auto`) |
