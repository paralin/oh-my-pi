...
- Never blank a result below `50` tokens (`MIN_PRUNE_TOKENS`): the `[Output truncated - N tokens]` placeholder costs ~8 tokens, so pruning a sub-floor result would grow the context and churn the prompt cache for nothing.
...
- **Per-turn useless-result pass** (`pruneUselessToolResults`, gated by `compaction.dropUseless`, default on): flagged results are blanked to `[Uneventful result elided]` (`USELESS_NOTICE`) only when the suffix is small (≤ ~8k tokens) or the provider cache has expired. Results smaller than the notice itself are never blanked.
- **Threshold prune** (`pruneToolOutputs`): flagged results bypass the protect-recent window and receive `USELESS_NOTICE` instead of the token-count placeholder.
...
`packages/agent/src/compaction/compaction.ts` also exports `generateHandoff(...)`. Handoff generation uses the same `completeSimple(...)` one-shot style as summarization, preserves the live agent cache prefix with the active system prompt, fixed IPython interface, and LLM message history, then appends one agent-attributed `user` message containing the handoff prompt. It accepts text only and returns joined text blocks directly.
